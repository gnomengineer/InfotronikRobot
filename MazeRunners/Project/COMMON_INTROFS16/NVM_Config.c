...
uint8_t NVMC_SaveTurnData(void *data, uint16_t dataSize) {
	if (dataSize > NVMC_STEPS_DATA_SIZE) {
		return ERR_OVERFLOW;
	}
	return IFsh1_SetBlockFlash(data,
			(IFsh1_TAddress) (NVMC_STEPS_DATA_START_ADDR), dataSize);
}

void *NVMC_GetTurnData(void) {
	if (isErased((uint8_t*) NVMC_STEPS_DATA_START_ADDR, NVMC_STEPS_DATA_SIZE)) {
	return NULL;
}
return (void*) NVMC_STEPS_DATA_START_ADDR;
}
*/
void NVMC_Init(void) {
/* nothing needed */
